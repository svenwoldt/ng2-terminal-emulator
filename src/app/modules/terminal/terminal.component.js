...
var TerminalComponent = (function () {
    function TerminalComponent(terminalService) {
        ...
        var config = this.terminalConfiguration(this.configName);
        this.prompt = terminalService.prompt(config);
        this.outputDelay = config.outputDelay;
        this.allowTypingWriteDisplaying = config.allowTypingWriteDisplaying;
        if (config.getTypeEffect) {
            config.getTypeEffect.then(function (effect) {
                _this.typeSound = effect;
            });
        }
    ...
    TerminalComponent.prototype.handlePaste = function (e) {
        this.commandLine += e.clipboardData.getData('text/plain');
    };
    ;
    TerminalComponent.prototype.keypress = function (keyCode) {
    ...
    ;
    TerminalComponent.prototype.previousCommand = function () {
    ...
    TerminalComponent.prototype.nextCommand = function () {
        if (this.commandIndex == -1) {
            return;
        }
    ...
    TerminalComponent.prototype.cleanNonPrintableCharacters = function (input) {
        return input.replace(this.re, '');
    };
    ;
    TerminalComponent.prototype.execute = function () {
        var command = this.cleanNonPrintableCharacters(this.commandLine);
        this.commandLine = '';
        if (!command)
            return;
        ...
        this.results.push({ type: 'text', text: [this.prompt.text + command] });
    };
